...
# delete, overwrite or install into one. That includes `sample-vault`: it is only *meant*
# to hold invented practice data, and the docs say so repeatedly, but "they were told not
# to" is not a reason to destroy a vault on their behalf. This program has no recovery
# path, so the rule is the blunt one -- see a marker, leave the directory alone.
...
if ($InstallDir -ne $DefaultInstallDir -and (Test-IsVaultisInstall $DefaultInstallDir)) {
    ...
    if ($Answer -match '^[Yy]') {
        ...
        foreach ($Name in $OwnedNames) {
            ...
            # The guard, applied to every directory before it is deleted rather than to a
            # hardcoded exception list -- so a vault someone created in there under any
            # name survives, not just the one called sample-vault.
            if (Test-LooksLikeVault $Victim) {
                $KeptVaults += $Name
                continue
            }
